...
cards:
  ...
  - title: Col
    link: /docs/components/col
    description: A `col` is a flexible column container that can be used for structuring layouts. It inherits most properties from `box`, except for the `direction` property.
  - title: Field
...
User interfaces of apps created with `NubeSDK` are made from components that adapt to the store theme automatically, maintaining the design consistency and can be developed with JSX or declarative functions.
