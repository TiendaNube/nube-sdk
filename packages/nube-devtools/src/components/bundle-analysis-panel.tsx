import { AlertCircle, AlertTriangle, Info, FileCode, Package, ExternalLink, XCircle, CheckCircle2, FileWarning, RefreshCw } from "lucide-react"
...
function SeverityBadge({ severity }: { severity: AnalysisWarning["severity"] }) {
  const variants = {
  ...
  return (
    <Badge className={cn("uppercase text-[10px] font-mono", variants[severity])}>
      {severity}
    </Badge>
  )
}
...
function StatusBadge({ status }: { status: AnalysisReport["status"] }) {
  const config = {
    ok: { icon: CheckCircle2, text: "OK", className: "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" },
    parse_error: { icon: FileWarning, text: "Parse Error", className: "bg-red-500/20 text-red-400 border-red-500/30" },
    fetch_error: { icon: XCircle, text: "Fetch Error", className: "bg-red-500/20 text-red-400 border-red-500/30" },
  }
      ...
      <AccordionContent className="pb-4">
        <div className="pl-7 space-y-3">
          ...
          <div className="bg-zinc-800/50 rounded-md p-3 border border-zinc-700/50">
            <div className="text-xs text-zinc-400 uppercase tracking-wide mb-2 font-medium">Location</div>
              ...
              {warning.location.generated && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <code className="text-zinc-300 font-mono text-xs">
                    Line {warning.location.generated.line}, Col {warning.location.generated.column}
                  </code>
                </div>
              )}
              {warning.location.original ? (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <code className="text-emerald-400 font-mono text-xs">
                    {warning.location.original.source}:{warning.location.original.line}:{warning.location.original.column}
                  </code>
                </div>
              ) : (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="space-y-2 min-w-0 flex-1">
          <div className="text-sm font-medium text-red-400">
            {status === "fetch_error" ? "Failed to fetch bundle" : "Failed to parse bundle"}
          </div>
...
export function BundleAnalysisPanel({ analysis, onRetryAnalysis }: BundleAnalysisPanelProps) {
  if (!analysis) {
    return (
  ...
  return (
    <div className="space-y-6">
      ...
      {analysis.error && (
        <ErrorDisplay error={analysis.error} status={analysis.status} onRetry={onRetryAnalysis} />
      )}

      {/* Bundle Info */}
      <div className="bg-zinc-800/30 rounded-lg p-4 border border-zinc-700/50">
        <div className="text-xs text-zinc-400 uppercase tracking-wide mb-3 font-medium">Bundle Info</div>
          ...
          {"sourceMapResolved" in analysis.bundle && (
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className={cn(
                  "text-sm font-medium",
                  analysis.bundle.sourceMapResolved ? "text-emerald-400" : "text-zinc-500"
                )}>
                  {analysis.bundle.sourceMapResolved ? "Resolved" : "Not available"}
                </span>
              </div>
      ...
      {analysis.status === "ok" && (
        <div className="bg-zinc-800/30 rounded-lg p-4 border border-zinc-700/50">
          <div className="text-xs text-zinc-400 uppercase tracking-wide mb-3 font-medium">Summary</div>
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-zinc-300">
                <span className="font-semibold text-red-400">{errorCount}</span> errors
              </span>
            </div>
            <div className="flex items-center gap-2">
              <AlertTriangle className="size-4 text-yellow-400" />
              <span className="text-sm text-zinc-300">
                <span className="font-semibold text-yellow-400">{warningCount}</span> warnings
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Info className="size-4 text-blue-400" />
              <span className="text-sm text-zinc-300">
                <span className="font-semibold text-blue-400">{infoCount}</span> info
              </span>
            </div>
          </div>
