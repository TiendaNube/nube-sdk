...
export function useConsoleEvents() {
	const { events, setEvents } = useConsoleEventsContext();
	...
	useEffect(() => {
		chrome.scripting.executeScript({
			...
			world: "MAIN",
			func: () => {
				if (!window.__nubeSdkListener) {
					window.__nubeSdkListener = (event) => {
				...
				for (const app of window.__NUBE_SDK_APPS__) {
					app.worker.addEventListener("message", window.__nubeSdkListener);
				}
			},
		});
					...
					if (window.__nubeSdkListener) {
						for (const app of window.__NUBE_SDK_APPS__) {
							app.worker.removeEventListener(
								"message",
								window.__nubeSdkListener,
							);
						}
						// biome-ignore lint/performance/noDelete: <explanation>
						delete window.__nubeSdkListener;
					}
				},
			});
		};
