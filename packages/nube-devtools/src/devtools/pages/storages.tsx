...
function RecordList({
	...
	if (records.length === 0) {
		// Two different empty states: nothing stored at all is a page problem,
		// while a filter that matches nothing is the user's own doing and a
		// "Reload page" button would be the wrong advice.
		if (total === 0) {
			return (
				<div className="flex h-full flex-col items-center justify-center gap-2 px-4 text-center">
					<p className="text-sm">No storage data found</p>
					<p className="text-xs text-muted-foreground max-w-xs">
						Nothing has been persisted through NubeSDK on this page yet.
					</p>
					<Button
						variant="outline"
						size="sm"
					...
					return (
						<li key={record.id}>
							<button
								...
								className={`flex w-full items-center gap-2 px-3 py-2 text-left border-b hover:bg-accent/50 ${
									isSelected
		...
		<div className="flex h-full flex-col">
			...
			<div className="flex-1 overflow-y-auto p-3 space-y-2">
				<div className="text-[11px] text-muted-foreground">
					APP <span className="text-foreground">{record.appId}</span>
				</div>
				...
				<textarea
					...
					className="w-full min-h-40 resize-y rounded border bg-transparent p-2 font-mono text-xs outline-none focus-visible:border-ring"
				/>
